...
function GetSalt(req, res, email) {
    ...
    dbCon.query(sql, function(err, results) {
        ...
        if (results[0][0] === undefined) {
            ...
            res.render('customerlogin', {message: "User '" + email + "' not found"});
        } else {
...
function CheckCredentials(req, res, salt, email)
{
    ...
    dbCon.query(sql, function(err, results) {
        ...
        if (results[0][0] === undefined || results[0][0].result == 0) {
            console.log("customerlogin.js: No login credentials found");
            res.render('customerlogin', {message: "Password not valid for user '" + email + "'.  Please log in again."});
        } else {
            console.log("customerlogin.js: Credentials matched");
