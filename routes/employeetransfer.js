...
function RemoveFunds(req, res, balance)
{
  ...
  if(fromAccount == "CA")
  {
    if(amount > balance)
    {
      ...
      res.render('employeetransfer', {message: "Checking Account has: '" + balance + "' - Not Enough"});
    }
  ...
  else if (fromAccount == "SA")
  {
    if(amount > balance)
    {
      ...
      res.render('employeetransfer', {message: "Savings Account has: '" + balance + "' - Not Enough"});
    }
  ...
  else
  {
    ...
    res.render('employeetransfer', {message: "From: '" + fromAccount + "'"});
  }
}
...
function GetTransferHistory(req, res)
{
  ...
  dbCon.query(sql, function(err, result, rows, fields) {
      ...
      if (rows[0][0] === undefined) {
          ...
          res.render('customertransfer', {message: "User_role_id: '" + 1 + "' not found"});
      } else {
        console.log(result);
        objForEmpTransferEJS.result = result;
        RenderIt(req, res);
      }
  });
}
...
{
  res.render('employeetransfer', objForEmpTransferEJS);
}
